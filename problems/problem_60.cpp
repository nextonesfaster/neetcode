    ...
    TreeNode *deserialize(std::string data)
    {
        ...
        while (i < order.size())
        {
            ...
            if (!queue.empty())
            {
                ...
                TreeNode *right = (i + 1 < order.size() && order[i + 1].has_value()) ? new TreeNode(order[i + 1].value()) : nullptr;
                queue.push(right);
                parent->right = right;
                i += 2;
            }
    ...
    TreeNode *deserialize(std::string data)
    {
        ...
        while (i < order.size())
        {
            ...
            if (!queue.empty())
            {
                ...
                queue.push(maybe_left);
                parent->left = maybe_left;
                TreeNode *right = (i + 1 < order.size() && order[i + 1] != "n") ? new TreeNode(std::stoi(order[i + 1])) : nullptr;
                queue.push(right);
                parent->right = right;
                i += 2;
            }
...
{
    Codec codec;
    TreeNode *root = new TreeNode(4, new TreeNode(5), new TreeNode(7, new TreeNode(3), new TreeNode(10)));
