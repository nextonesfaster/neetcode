...
class Solution
{
    ...
    void dfs(int r, int c, std::unordered_set<int> &set, int prev_height, const std::vector<std::vector<int>> &heights)
    {
        int coord = r * COLS + c;
        if (r < 0 || r >= ROWS || c < 0 || c >= COLS || set.count(coord) || heights[r][c] < prev_height)
            return;
    ...
    void bfs(std::queue<std::pair<int, int>> &queue, std::vector<std::vector<uint8_t>> &visited, std::vector<std::vector<int>> &heights)
    {
        while (!queue.empty())
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (x >= 0 && x < ROWS && y >= 0 && y < COLS && visited[x][y] == 0 && heights[r][c] <= heights[x][y])
                    queue.push({x, y});
            }
        }
    }
